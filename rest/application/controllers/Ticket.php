...
class Ticket extends REST_Controller
{
    ...
    public function addTicket_post()
    {
        ...
        $this->form_validator->add_rules('issue', array('required' => $this->lang->line('ticke_issue_req')));
    ...
    public function addTicketChat_post()
    {
       ...
       // $this->form_validator->add_rules('message', array('required' => $this->lang->line('message_req')));
        ...
        $chat_data=array(
            ...
            'actions_names'=>!empty($action_message)?$action_message:''
        ...
        if(!empty($_FILES) && count($document_id)>0){
            ...
            $this->User_model->update_data('ticket',array('last_updated_by'=>!empty($this->session_user_id)?$this->session_user_id:'0','last_update_on'=>currentDate(),'status'=>$data['status']),array('id'=>$data['ticket_id']));
