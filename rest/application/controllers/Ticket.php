...
class Ticket extends REST_Controller
{
    ...
    public function addTicket_post()
    {
       ...
       if(!empty($_FILES) && count($document_id)>0){
           ...
           if($inserted_id>0){
            ...
            if($template_configurations['total_records']>0){
                ...
                $notification_comments = wildcardreplace($template_configurations['wildcards'],$notification_wildcards_replaces,$template_configurations['notification_comments']);
            ...
            $user_ids_send_mails=array(1); //this is used for send mails for  specific admins to pass user id 
       ...
       elseif(empty($_FILES)){
            ...
            $user_ids_send_mails=array(1); //this is used for send mails for  specific admins to pass user_role id 
            foreach($user_ids_send_mails as $k=>$v){
    ...
    public function addTicketChat_post()
    {
       ...
       // $this->form_validator->add_rules('message', array('required' => $this->lang->line('message_req')));
        ...
        if(!empty($_FILES) && count($document_id)>0){
            ...
            $ticket_info=$this->Ticket_model->getTicketData(array('ticket_id'=>$data['ticket_id']));//print_r($ticket_info);exit;
            if($this->session_user_info->user_id==$ticket_info[0]['ticket_rised_by']){//check the login user is ticket rised user or not for send maisl to admin user
            ...
            foreach($user_ids_send_mails as $k=>$v){
                $user_details=$this->User_model->check_record_selected('id as user_id,concat(first_name," ",last_name) as user_name,email','user',array('id'=>$v));//echo $this->db->last_query($user_details);exit;
                ...
                if($template_configurations['total_records']>0){
                    ...
                    $wildcards_replaces['ticket_no']=$ticket_info[0]['issue_id'];
                    $wildcards_replaces['ticket_title']=$ticket_info[0]['title'];
                    $wildcards_replaces['comments']=!empty($data['message'])?$data['message']:'';
